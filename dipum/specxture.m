...
%   angle for 0 to 180 degrees in increments of 1 degree, and S = log(1
%   + spectrum of f), normalized to the range [0,1]. The maximum value
%   of radius is min(M,N), where M and N are the number of rows and
%   columns of image (region) f. Thus, SRAD is a row vector of length =
%   (min(M,N)/2) - 1; and SANG is a row vector of length 180.
%
...
% Obtain the centered spectrum, S, of f. The variables of S are (u,v),
% running from 1:M and 1:N, with the center (zero frequency) at [M/2 +
% 1,N/2 + 1] (see Chapter 4).
S = fftshift(fft2(f));
S = abs(S);
[M,N] = size(S);
x0 = M/2 + 1;
...
% Maximum radius that guarantees a circle centered at (x0,y0) that does
% not exceed the boundaries of S.
rmax = min(M,N)/2 - 1; 
...
%  from (x0,y0) to (x,y). 
%
%  Based on utility function intline.m.  xr and yr are returned as
%  column vectors.
