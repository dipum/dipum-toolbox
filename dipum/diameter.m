...
%   of Digital Image Processing, by Gonzalez and Woods, 4th edition,
...
for k = 1:length(s)
   ...
   [s(k).BasicRectangle, s(k).MinorAxis] = ...
      computeBasicRectangle(s(k),perim_r,perim_c);
end

%----------------------------------------------------------------------%
function [d,majoraxis,r,c] = computeDiameter(s)
%  [D,MAJORAXIS,R,C] = computeDiameter(S) computes the diameter and
%  major axis for the region represented by the structure S. S must
%  contain the fields Image and BoundingBox.  computeDiameter also
...
%----------------------------------------------------------------------%
function [basicrect,minoraxis] = computeBasicRectangle(s, ...
                                                    perim_r, perim_c)
%   [BASICRECT,MINORAXIS] = computeBasicRectangle(S, PERIM_R, PERIM_C)
%   computes the basic rectangle and the minor axis end-points for the
%   region represented by the structure S.  S must contain the fields
%   Image, BoundingBox, MajorAxis, and Diameter. PERIM_R and PERIM_C are
%   the row and column coordinates of perimeter of s.Image. BASICRECT is
%   a 4-by-2 matrix, each row of which contains the row and column
%   coordinates of one corner of the basic rectangle.
...
%----------------------------------------------------------------------%
function [r,c] = prunePixelList(r,c)
%   [R,C] = prunePixelList(R,C) removes pixels from the vectors R and C
%   that cannot be endpoints of the major axis.  This elimination is
%   based on geometrical constraints described in Russ, Image Processing
%   Handbook, 2007, Chapter 8.
