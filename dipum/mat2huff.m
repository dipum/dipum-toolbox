...
%   in unit-width histogram bins between X's minimum and maximum values.
...
%   integer values, it can be input directly to MAT2HUFF. The minimum
%   value of X must be representable as an int16.
%
%   If X is double with non-integer values---for example, an image with
%   values between 0 and 1---first scale X to an appropriate integer
%   range before the call. For example, use Y = MAT2HUFF(255*X) for 256
%   gray level encoding.
%
%   NOTE: The number of Huffman code words is round(max(X(:))) -
%   round(min(X(:))) + 1.  You may need to scale input X to generate
%   codes of reasonable length.  The maximum row or column dimension of
%   X is 65535.
%
%   See also HUFF2MAT.
%
...
% Compute the input histogram between xmin and xmax with unit width
% bins, scale to UINT16, and store.
x = x(:)';
h = histcounts(x,xmin:xmax+1);
if max(h) > 65535
    h = 65535 * h / max(h);
end
h = uint16(h);   y.hist = h;
